'use strict';
moduleComponent.component('episodioSelection', {
    templateUrl: 'app/episodio/selection.html',
    controllerAs: 'c',
...
function cController($http) {
    ...
    $http({
        ...
        self.ajaxDataEpisodiosNumber = response.data.message;
        self.totalPages = Math.ceil(self.ajaxDataEpisodiosNumber / self.rpp);
        if (self.page > self.totalPages) {
            self.page = self.totalPages;
        }
        pagination();
    }, function (response) {
        self.ajaxDataEpisodiosNumber = response.data.message || 'Request failed';
        self.status = response.status;
    });
    ...
    self.save = function (id, fecha_inicio, fecha_alta) {
        self.obj={id : id,fecha_inicio : fecha_inicio,fecha_alta : fecha_alta};
    };
